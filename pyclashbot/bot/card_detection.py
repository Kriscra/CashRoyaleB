...
def _normalize_histogram(counts: numpy.ndarray) -> numpy.ndarray:
    counts = counts.astype(float, copy=False)
    total = float(counts.sum())
    if total <= 0:
        return numpy.zeros_like(counts, dtype=float)
    return counts / total


CARD_COLOR_DATA_NORMALIZED: dict[str, numpy.ndarray] = {}

for card_name, card_data in card_color_data.items():
    card_arrays = [numpy.array(list(corner.values()), dtype=float) for corner in card_data]
    card_color_data[card_name] = card_arrays
    CARD_COLOR_DATA_NORMALIZED[card_name] = numpy.stack(
        [_normalize_histogram(array) for array in card_arrays],
    )


def calculate_offset(card_name, card_data, collected_data_array):
    total_offset = float(numpy.sum(numpy.abs(collected_data_array - card_data)))
    return card_name, total_offset


NORMALIZED_ACCEPTANCE_THRESHOLD = 3.0
NORMALIZED_MARGIN_THRESHOLD = 0.4


def find_closest_card(collected_data):
    best_card: str | None = None
    best_offset = float("inf")
    second_best_offset = float("inf")

    collected_data_array = numpy.array(
        [
            _normalize_histogram(numpy.array(list(corner.values()), dtype=float))
            for corner in collected_data
        ],
    )

    for card_name, card_data in CARD_COLOR_DATA_NORMALIZED.items():
        ...
        if total_offset < best_offset:
            ...
            best_card = card_name
        elif total_offset < second_best_offset:
            second_best_offset = total_offset

    if best_card is None:
        return "UNKNOWN"

    if best_offset <= NORMALIZED_ACCEPTANCE_THRESHOLD:
        return best_card

    if second_best_offset - best_offset >= NORMALIZED_MARGIN_THRESHOLD:
        return best_card
